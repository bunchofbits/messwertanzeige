...
static void port_setup(int fd, struct termios *oldterm, struct conf *cnf )
{
  struct termios *term;
...
static int read_version(int fd, int output)
{
  ...
  clr_rts(fd);
  write(fd,"V",1);
  tcdrain(fd);			/* warten, dass alles geschrieben */
  set_rts(fd);
  sleep(1);			/* auf sonde warten */
  ...
  if (!ret) {
    if(output > 0)
      fprintf(stderr,"No version within timeout.\n");
    exit(1);
  }
  ...
  ret = 0;
  cnt = 0;
  while(cnt < 200 && ret < 21)
    {
      ret += read(fd, &buf[ret], 21 - ret);
      if(output > 0 && verbose == 2)
	printf("%s: ret: %d\tbuf: '%s'\n",__func__, ret, buf);
      cnt++;
    }
  if(cnt == 200 && ret < 21)
    {
      if(output >0)
	fprintf(stderr,"reading failed after 200 trys\n");
      return -1;
    }
...
int main(int argc,char *argv[]) {
  ...
  struct conf config={ .wait=60, .version=0, .speed=0 };
